...
class FileIOOps:
    ...
    def compile_read_text_file(self, node):
        """Compile ReadTextFile (syscall-only, lseek sizing, never-NULL, callee-saved)"""
        if not node.arguments or len(node.arguments) != 1:
            raise ValueError("ReadTextFile requires exactly one argument")

        path_arg = node.arguments[0]

        # ---- Resolve path into RSI (use your existing literal/identifier handling) ----
        ...
        # Save RBX (callee-saved); we’ll hold size there
        self.compiler.asm.emit_push_rbx()

        # open(path, O_RDONLY, 0)
        self.compiler.asm.emit_mov_rdi_rsi()
        self.compiler.asm.emit_push_rax()
        self.compiler.asm.emit_xor_eax_eax()
        self.compiler.asm.emit_mov_rsi_rax()        # O_RDONLY = 0
        self.compiler.asm.emit_xor_edx_edx()        # mode = 0
        ...
        self.compiler.asm.emit_jump_to_label(err_open, "JL")   # fd < 0 → error

        # Save fd on stack
        self.compiler.asm.emit_push_rax()                      # [fd]

        # === size = lseek(fd, 0, SEEK_END); rewind to 0 ===
        self.compiler.asm.emit_mov_rdi_mem_offset('RSP', 0)    # fd
        self.compiler.asm.emit_xor_esi_esi()                   # offset=0
        self.compiler.asm.emit_mov_rdx_imm64(2)                # SEEK_END
        self.compiler.asm.emit_mov_rax_imm64(8)                # sys_lseek
        self.compiler.asm.emit_syscall()                       # RAX=size or <0
        self.compiler.asm.emit_test_rax_rax()
        ...
        self.compiler.asm.emit_pop_rax()                       # discard size
        # stack: [fd]
        ...
        self.compiler.asm.emit_mov_rdx_rbx()                   # size
        self.compiler.asm.emit_mov_rdi_mem_offset('RSP', 16)   # fd
        self.compiler.asm.emit_xor_eax_eax()                   # sys_read (0)
        self.compiler.asm.emit_syscall()                       # RAX=bytes_read or <0

        self.compiler.asm.emit_test_rax_rax()
        self.compiler.asm.emit_jump_to_label(ok_read, "JGE")   # ok if bytes_read >= 0

        # -------------------- err_read (after setting up [dummy][buffer][fd]) --------------------
        self.compiler.asm.mark_label(err_read)
        self.compiler.asm.emit_add_rsp_imm32(24)               # drop [dummy][buffer][fd]
        # fall through to common "return empty string" sequence
        # ----------------------------------------------------------------------------------------

        # -------------------- err_seek (only [fd] on stack) -------------------------------------
        self.compiler.asm.mark_label(err_seek)
        # Close fd to be tidy (optional but harmless)
        self.compiler.asm.emit_mov_rdi_mem_offset('RSP', 0)    # fd
        ...
        # cleanup & return
        self.compiler.asm.emit_add_rsp_imm32(8)                # drop [fd]
        self.compiler.asm.emit_mov_rsi_rax()                   # RSI = buffer for downstream
        self.compiler.asm.emit_pop_rbx()                       # restore RBX
        self.compiler.asm.emit_push_rax()                      # return buffer
        self.compiler.asm.emit_jump_to_label(end_lbl, "JMP")
